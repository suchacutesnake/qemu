...
{
    qemu_set_irq(s->irq, (s->iflag1 & s->imask1) != 0);
}
...
static void flexcan_receive_one(FslFlexCANState *s, const qemu_can_frame *frame)
{
    ...
    for (mb = 0; mb <= flexcan_last_mb(s); mb++) {
        if (flexcan_match_frame(s, mb, frame)) {
            flexcan_write_frame_to_mb(s, mb, frame);
            return;
        }
    }
}
...
static uint64_t flexcan_read(void *opaque, hwaddr offset, unsigned size)
{
    ...
    default:
        if (offset >= FLEXCAN_MB_BASE && offset < FLEXCAN_MB_END) {
            return flexcan_mb_get_word(s, offset);
        }
                          ...
                          uint64_t value, unsigned size)
{
    ...
    default:
        if (offset >= FLEXCAN_MB_BASE && offset < FLEXCAN_MB_END) {
            ...
            if ((offset - FLEXCAN_MB_BASE) % flexcan_mb_stride(s) == 0) {
                int mb = (offset - FLEXCAN_MB_BASE) / flexcan_mb_stride(s);
                flexcan_do_tx(s, mb);
            }
